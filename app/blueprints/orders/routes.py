...
def create():
    ...
    if request.method == 'GET':
        ...
        # Шаблон ждёт объекты с .menu_item и .quantity — собираем лёгкие записи,
        # чтобы не плодить временные OrderItem в сессии SQLAlchemy.
        order_items = [
        ...
        return render_template('orders/create.html', order_items=order_items, total_price=total_price)
    ...
    if reserve:
        return redirect(url_for('reservations.select_table', order_id=order.id))
